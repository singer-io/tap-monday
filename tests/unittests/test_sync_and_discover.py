...
class TestSchemaAdditionalProperties(unittest.TestCase):
    """Test that root-level schemas do not contain additionalProperties: true."""
    ...
    def test_no_root_additional_properties(self, stream_name):
        """Schema root level must not have additionalProperties: true."""
        schema_path = SCHEMAS_DIR / f"{stream_name}.json"
        with open(schema_path) as f:
            schema = json.load(f)
        self.assertNotEqual(
            schema.get("additionalProperties"),
            True,
            f"Schema '{stream_name}.json' should not have additionalProperties: true at root level"
        )


class TestBoardItemsDescriptionSchema(unittest.TestCase):
    """Test that board_items description field is typed as string, not object."""

    def test_description_is_string_type(self):
        """board_items description should be a string type, not a nested object."""
        schema_path = SCHEMAS_DIR / "board_items.json"
        with open(schema_path) as f:
            schema = json.load(f)

        description_schema = schema["properties"]["description"]
        types = description_schema["type"]
        if isinstance(types, str):
            types = [types]
        self.assertIn("string", types,
                      "board_items description field should be of string type")
        self.assertNotIn("object", types,
                         "board_items description field should not be of object type")
        self.assertNotIn("properties", description_schema,
                         "board_items description field should not have nested properties")
