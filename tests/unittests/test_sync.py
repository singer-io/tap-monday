...
def _build_fake_streams(*specs, synced_list: list):
    """Build a STREAMS-like dict from a sequence of ``(name,)``,
    ``(name, parent)`` or ``(name, parent, children)`` tuples, wiring every
    stream class to *synced_list* for call tracking."""
    result = {}
    for spec in specs:
        if len(spec) == 1:
...
def _transformer_patch():
    """Return a context-manager-compatible Transformer mock."""
    m = MagicMock()
